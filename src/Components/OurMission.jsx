  ...
  return (
    <div className="flex sm:flex-col-reverse md:flex-row justify-between gap-2 max-w-7xl mx-auto mt-8 mb-14 px-4">
                    ...
                    missions.map((v , i)=>(
                        <HoverCard key={i} dropped={i%2!==0}>
                            <div className="flex flex-col max-w-64 space-y-4">
                                <img className='size-16' src={v.img} alt={v.title.toLowerCase()} />
        ...
        <div className="flex flex-col sm:items-center items-end">
            ...
            <p className='w-80 sm:text-center text-end'>The Cybersecurity Student Community (CYSCOM). VITCC aims to spread awareness about cybersecurity.</p>
        </div>
    </div>
  )
}
